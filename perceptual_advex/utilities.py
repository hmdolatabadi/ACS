...
from cords.utils.models import ResNet50_PercepAdvex
    ...
    if arch.startswith('rob-') or (
        ...
        if (
            ...
            if checkpoint_fname == 'pretrained':
                ...
                #     num_classes=12)
                model   = ResNet50_PercepAdvex(num_classes=12)
        ...
        if checkpoint_fname is not None and checkpoint_fname != 'pretrained':
            try:
                ...
                # for key_old, key_new in zip(state['model'].keys(), model.state_dict().keys()):
                #     new_state_dict[key_new] = state['model'][key_old]
