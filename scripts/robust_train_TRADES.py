...
from cords.utils.custom_dataset import load_dataset_custom, CustomTradesAdvDataset, cifar10_mean, cifar10_std, clamp
from torch.utils.data import Subset
from cords.utils.config_utils import load_config_data
import os.path as osp
from cords.robustselectionstrategies.supervisedlearning import OMPGradMatchStrategy, RandomStrategy, CRAIGStrategy
...
# TODO: Mix the code for all different objectives in a signle python file.
# This way, instead of having four different files for different objectiv-
# es we'd have one.
class TrainRobustClassifier:
    ...
    def create_model(self):
        ...
        elif self.configdata['model']['architecture'] == 'PreActResNet18':
            model = PreActResNet18(self.configdata['model']['numclasses'])
    ...
    # Borrowed from TRADES GitHub repo: https://github.com/yaodongyu/TRADES
    def trades_adver(self,
                    model,
                    inputs,
                    alpha=1,
                    epsilon=8,
        ...
        inputs_adv = inputs.detach() + 0.001 * torch.randn(inputs.shape).cuda().detach()

        if distance == 'l_inf':
            for _ in range(attack_iters):
                inputs_adv.requires_grad_()
                with torch.enable_grad():
                    loss_kl = criterion_kl(F.log_softmax(model(inputs_adv), dim=1),
                                           F.softmax(model(inputs), dim=1))
                grad       = torch.autograd.grad(loss_kl, [inputs_adv])[0]
                inputs_adv = inputs_adv.detach() + alpha * torch.sign(grad.detach())
                inputs_adv = clamp(inputs_adv, inputs - epsilon, inputs + epsilon)
                inputs_adv = clamp(inputs_adv, lower_limit, upper_limit)

        elif distance == 'l_2':
            delta = 0.001 * torch.randn(inputs.shape).cuda().detach()
            delta.requires_grad = True

            # Setup optimizers
            optimizer_delta = optim.SGD([delta], lr=epsilon / attack_iters * 2)
            ...
            for _ in range(attack_iters):
                ...
                delta.data = clamp(delta.data, lower_limit, upper_limit)
                delta.data.sub_(inputs)
                delta.data.renorm_(p=2, dim=0, maxnorm=epsilon)

            inputs_adv = inputs + delta.detach()
            inputs_adv.requires_grad = False
        else:
            inputs_adv = clamp(inputs, lower_limit, upper_limit)
    ...
    def get_perturbations(self, model, optimizer, dataloader, criterion, epsilon, alpha, attack_iters, delta_init, std, attack_type='TRADES'):
        ...
        if attack_type == 'TRADES':
            ...
            for batch_idx, (inputs, targets) in enumerate(dataloader):
                ...
                inputs_adv = self.trades_adver(model=model,
                                               inputs=input_var,
    ...
    def train(self):
        ...
        trainadvset = CustomTradesAdvDataset(trainset)
        validadvset = CustomTradesAdvDataset(validset)
        ...
        if self.configdata['dss_strategy']['type'] == 'GradMatch':
            ...
            setf_model = OMPGradMatchStrategy(trainadvloader, valadvloader, model1, criterion_nored,
                                              ...
                                              beta=self.configdata['train_args']['beta'])
        ...
        for i in range(start_epoch, self.configdata['train_args']['num_epochs']):
            ...
            if self.configdata['dss_strategy']['type'] in ['CRAIG', 'CRAIGPB', 'GradMatch', 'GradMatchPB']:
                ...
                for batch_idx, (inputs, targets) in enumerate(subset_trnloader):
                    ...
                    loss_natural = criterion_nored(outputs, targets)
                    loss_robust  = criterion_kl_nored(F.log_softmax(model(inputs_adv), dim=1),
                                                      F.softmax(model(inputs), dim=1)).sum(dim=1)

                    losses = loss_natural + self.configdata['train_args']['beta'] * loss_robust
            ...
            if self.configdata['dss_strategy']['type'] in ['CRAIGPB-Warm', 'CRAIG-Warm', 'GradMatch-Warm', 'GradMatchPB-Warm']:
                pass_eval = (i>=full_epochs) and (i<kappa_epochs)

            if ((i+1) % self.configdata['train_args']['print_every'] == 0) and not pass_eval:
    ...
    def eval(self):
        ...
        if True:
            ...
            for _, (inputs, targets) in enumerate(testloader):
                inputs, targets  = inputs.to(self.configdata['train_args']['device']), targets.to(self.configdata['train_args']['device'], non_blocking=True)
                pgd_delta        = self.attack_pgd(model, inputs, targets, criterion, criterion_nored, epsilon, alpha, attack_iters=50, restarts=20)  # TODO
